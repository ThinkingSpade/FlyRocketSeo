...
// Dated "what we did" journal entries per project (content published, redirects
// fixed, site migrated…). Rendered as markers on rank-trend charts so ranking
// movements can be correlated with the actions that caused them.
export const projectEvents = sqliteTable(
  "project_events",
  {
    ...
    projectId: text("project_id")
      .notNull()
      .references(() => projects.id, { onDelete: "cascade" }),
    // Day precision ("YYYY-MM-DD"): chart markers and the event list only need
    // dates, and ISO date strings compare lexicographically = chronologically.
  ...
  (table) => [
    // Charts fetch events per project ordered/bounded by date.
    index("project_events_project_date_idx").on(
      table.projectId,
      table.eventDate,
    ),
  ],
