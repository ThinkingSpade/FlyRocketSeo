...
// `formatAxisValue`/`formatChartTick` stay local: the ECharts axes need the
// one-fraction-digit compact format documented below, which the Recharts-era
// helpers in BacklinksChartChrome do not apply.
import {
  EmptyChartState,
  isActivitySeries,
  isStableSeries,
...
export function BacklinksAuthorityChart({
  ...
  const options = useMemo(
    () => ({
      ...base,
      ...legendAndGrid(base, theme, ["Domain authority"]),
      tooltip: { ...base.tooltip, ...TOOLTIP },
  ...
  return (
    <div className="h-56 min-w-0" aria-label="Domain authority trend chart">
      <Chart
        echarts={echarts}
        options={options}
        height={CHART_HEIGHT}
